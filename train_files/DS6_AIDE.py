...
from utils import CrossEntropyLoss2d, DiceLoss, MulticlassDiceLoss, CEMDiceLossImage, PolyLR, \
    MulticlassDice_fn, MulticlassAccuracy_fn, Dice_fn, MulticlassMSELoss,segmentationLossImage, consistencyLossImage
...
# def seg_loss(inputs, targets, smooth=1, alpha=1):
#     inputs = inputs[:, 1]
#     # dice loss
#     dice_inputs = torch.sigmoid(inputs)
#     # flatten
#     iflat = dice_inputs.reshape(-1)
#     tflat = targets.reshape(-1)
#
#     intersection = (iflat * tflat).sum()
#     dice = (2. * intersection + smooth) / (iflat.sum() + tflat.sum() + smooth)
...
#
#     return dice_loss + (alpha * BCE_loss)
#
#
...
def Train(train_root, train_csv, test_csv, traincase_csv, testcase_csv, labelcase_csv, tempmaskfolder):
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        for batch_idx, (inputs, augset, targets, targets1, targets2) in \
            ...
            train_dice1 += get_dice_score(outputs1, targets2).item()
            train_loss2 += loss2.item() * inputs.shape[0]
            train_dice2 += get_dice_score(outputs2, targets1).item()
